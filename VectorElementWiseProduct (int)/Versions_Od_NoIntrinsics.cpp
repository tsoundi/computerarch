...
_int64 version_vectorization_and_loopenrolling(int n, int* arr, int* arr2, int* result) {
	...
	for (int i = 0; i < n; i += 32) {
		...
		__m256i vec3 = _mm256_load_si256((__m256i*) & arr[i + 8]);
		__m256i vec4 = _mm256_load_si256((__m256i*) & arr2[i + 8]);

		__m256i res2 = _mm256_mullo_epi32(vec3, vec4);

		_mm256_store_si256((__m256i*) & result[i + 8], res2);
		//#################################################################################

		__m256i vec5 = _mm256_load_si256((__m256i*) & arr[i + 16]);
		__m256i vec6 = _mm256_load_si256((__m256i*) & arr2[i + 16]);

		__m256i res3 = _mm256_mullo_epi32(vec5, vec6);

		_mm256_store_si256((__m256i*) & result[i + 16], res3);
		//#################################################################################

		__m256i vec7 = _mm256_load_si256((__m256i*) & arr[i + 24]);
		__m256i vec8 = _mm256_load_si256((__m256i*) & arr2[i + 24]);

		__m256i res4 = _mm256_mullo_epi32(vec7, vec8);

		_mm256_store_si256((__m256i*) & result[i + 24], res4);
